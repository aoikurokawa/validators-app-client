...
export function useHelloProgram() {
  ...
  const programId = useMemo(
    () => getHelloProgramId(cluster.network as Cluster),
    [cluster],
  );
  const program = useMemo(
    () => getHelloProgram(provider, programId),
    [provider, programId],
  );
  ...
  const initialize = useMutation({
    ...
    mutationFn: (keypair: Keypair) =>
      program.methods
        .initialize()
        .accounts({ hello: keypair.publicKey })
        .signers([keypair])
        .rpc(),
    onSuccess: (signature) => {
...
export function useHelloProgramAccount({ account }: { account: PublicKey }) {
  ...
  const closeMutation = useMutation({
    ...
    mutationFn: () =>
      program.methods.close().accounts({ hello: account }).rpc(),
    onSuccess: (tx) => {
      transactionToast(tx);
      return accounts.refetch();
    },
  ...
  const decrementMutation = useMutation({
    ...
    mutationFn: () =>
      program.methods.decrement().accounts({ hello: account }).rpc(),
    onSuccess: (tx) => {
      transactionToast(tx);
  ...
  const incrementMutation = useMutation({
    ...
    mutationFn: () =>
      program.methods.increment().accounts({ hello: account }).rpc(),
    onSuccess: (tx) => {
      transactionToast(tx);
      return accountQuery.refetch();
    },
  ...
  const setMutation = useMutation({
    ...
    mutationFn: (value: number) =>
      program.methods.set(value).accounts({ hello: account }).rpc(),
    onSuccess: (tx) => {
      transactionToast(tx);
      return accountQuery.refetch();
    },
